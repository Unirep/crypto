"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.SparseMerkleTreeImpl = void 0;
/* External Imports */
var assert = require("assert");
/* Internal Imports */
var crypto_1 = require("../crypto");
var SparseMerkleTreeImpl = /** @class */ (function () {
    function SparseMerkleTreeImpl(db, height) {
        this.db = db;
        this.height = height;
        assert(height > 0, 'SMT height needs to be > 0');
        this.numLeaves = BigInt(Math.pow(2, height));
    }
    SparseMerkleTreeImpl.create = function (db, height, zeroHash) {
        return __awaiter(this, void 0, void 0, function () {
            var tree;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        tree = new SparseMerkleTreeImpl(db, height);
                        return [4 /*yield*/, tree.init(zeroHash)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/, tree];
                }
            });
        });
    };
    SparseMerkleTreeImpl.prototype.init = function (zeroHash) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.populateZeroHashesAndRoot(zeroHash)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    SparseMerkleTreeImpl.prototype.getHeight = function () {
        return this.height;
    };
    SparseMerkleTreeImpl.prototype.getRootHash = function () {
        return this.root;
    };
    SparseMerkleTreeImpl.prototype.getZeroHash = function (index) {
        return this.zeroHashes[index];
    };
    SparseMerkleTreeImpl.prototype.update = function (leafKey, leafHash) {
        return __awaiter(this, void 0, void 0, function () {
            var nodeIndex, nodeHash, nodeHashString, isLeftNode, sibNodeIndex, sibNodeHash, parentNodeIndex, parentHash, i, sibNodeHashString;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        assert(leafKey < this.numLeaves, "leaf key " + leafKey + " exceeds total number of leaves " + this.numLeaves);
                        nodeIndex = leafKey.valueOf() + this.numLeaves.valueOf();
                        return [4 /*yield*/, this.db.get(nodeIndex.toString())];
                    case 1:
                        nodeHashString = _a.sent();
                        if (nodeHashString === leafHash.toString())
                            return [2 /*return*/];
                        else
                            nodeHash = leafHash;
                        return [4 /*yield*/, this.db.set(nodeIndex.toString(), nodeHash.toString())];
                    case 2:
                        _a.sent();
                        isLeftNode = nodeIndex % BigInt(2) === BigInt(0) ? true : false;
                        sibNodeIndex = isLeftNode ? nodeIndex + BigInt(1) : nodeIndex - BigInt(1);
                        i = 0;
                        _a.label = 3;
                    case 3:
                        if (!(i < this.height)) return [3 /*break*/, 7];
                        return [4 /*yield*/, this.db.get(sibNodeIndex.toString())];
                    case 4:
                        sibNodeHashString = _a.sent();
                        if (!sibNodeHashString)
                            sibNodeHash = this.zeroHashes[i];
                        else
                            sibNodeHash = BigInt(sibNodeHashString);
                        parentNodeIndex = nodeIndex / BigInt(2);
                        parentHash = isLeftNode ? crypto_1.newWrappedPoseidonT3Hash(nodeHash, sibNodeHash) : crypto_1.newWrappedPoseidonT3Hash(sibNodeHash, nodeHash);
                        return [4 /*yield*/, this.db.set(parentNodeIndex.toString(), parentHash.toString())];
                    case 5:
                        _a.sent();
                        nodeIndex = parentNodeIndex;
                        nodeHash = parentHash;
                        isLeftNode = nodeIndex % BigInt(2) === BigInt(0) ? true : false;
                        sibNodeIndex = isLeftNode ? nodeIndex + BigInt(1) : nodeIndex - BigInt(1);
                        _a.label = 6;
                    case 6:
                        i++;
                        return [3 /*break*/, 3];
                    case 7:
                        assert(nodeIndex === BigInt(1), "Root node index must be 1");
                        this.root = parentHash;
                        return [2 /*return*/];
                }
            });
        });
    };
    SparseMerkleTreeImpl.prototype.getMerkleProof = function (leafKey) {
        return __awaiter(this, void 0, void 0, function () {
            var siblingNodeHashes, nodeIndex, isLeftNode, sibNodeIndex, sibNodeHash, i, sibNodeHashString;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        assert(leafKey < this.numLeaves, "leaf key " + leafKey + " exceeds total number of leaves " + this.numLeaves);
                        siblingNodeHashes = [];
                        nodeIndex = leafKey.valueOf() + this.numLeaves.valueOf();
                        isLeftNode = nodeIndex % BigInt(2) === BigInt(0) ? true : false;
                        sibNodeIndex = isLeftNode ? nodeIndex + BigInt(1) : nodeIndex - BigInt(1);
                        i = 0;
                        _a.label = 1;
                    case 1:
                        if (!(i < this.height)) return [3 /*break*/, 4];
                        return [4 /*yield*/, this.db.get(sibNodeIndex.toString())];
                    case 2:
                        sibNodeHashString = _a.sent();
                        if (!sibNodeHashString)
                            sibNodeHash = this.zeroHashes[i];
                        else
                            sibNodeHash = BigInt(sibNodeHashString);
                        siblingNodeHashes.push(sibNodeHash);
                        nodeIndex = nodeIndex / BigInt(2);
                        isLeftNode = nodeIndex % BigInt(2) === BigInt(0) ? true : false;
                        sibNodeIndex = isLeftNode ? nodeIndex + BigInt(1) : nodeIndex - BigInt(1);
                        _a.label = 3;
                    case 3:
                        i++;
                        return [3 /*break*/, 1];
                    case 4:
                        assert(siblingNodeHashes.length == this.height, "Incorrect number of proof entries");
                        return [2 /*return*/, siblingNodeHashes];
                }
            });
        });
    };
    SparseMerkleTreeImpl.prototype.verifyMerkleProof = function (leafKey, proof) {
        return __awaiter(this, void 0, void 0, function () {
            var nodeIndex, nodeHash, nodeHashString, isLeftNode, _i, proof_1, sibNodeHash;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        assert(leafKey < this.numLeaves, "leaf key " + leafKey + " exceeds total number of leaves " + this.numLeaves);
                        assert(proof.length == this.height, "Incorrect number of proof entries");
                        nodeIndex = leafKey.valueOf() + this.numLeaves.valueOf();
                        return [4 /*yield*/, this.db.get(nodeIndex.toString())];
                    case 1:
                        nodeHashString = _a.sent();
                        if (!nodeHashString)
                            nodeHash = this.zeroHashes[0];
                        else
                            nodeHash = BigInt(nodeHashString);
                        isLeftNode = nodeIndex % BigInt(2) === BigInt(0) ? true : false;
                        for (_i = 0, proof_1 = proof; _i < proof_1.length; _i++) {
                            sibNodeHash = proof_1[_i];
                            nodeHash = isLeftNode ? crypto_1.newWrappedPoseidonT3Hash(nodeHash, sibNodeHash) : crypto_1.newWrappedPoseidonT3Hash(sibNodeHash, nodeHash);
                            nodeIndex = nodeIndex / BigInt(2);
                            isLeftNode = nodeIndex % BigInt(2) === BigInt(0) ? true : false;
                        }
                        if (nodeHash === this.root)
                            return [2 /*return*/, true];
                        else
                            return [2 /*return*/, false];
                        return [2 /*return*/];
                }
            });
        });
    };
    SparseMerkleTreeImpl.prototype.populateZeroHashesAndRoot = function (zeroHash) {
        return __awaiter(this, void 0, void 0, function () {
            var hashes, i;
            return __generator(this, function (_a) {
                hashes = [
                    zeroHash,
                ];
                for (i = 1; i < this.height; i++) {
                    hashes[i] = crypto_1.newWrappedPoseidonT3Hash(hashes[i - 1], hashes[i - 1]);
                }
                this.zeroHashes = hashes;
                this.root = crypto_1.newWrappedPoseidonT3Hash(hashes[this.height - 1], hashes[this.height - 1]);
                return [2 /*return*/];
            });
        });
    };
    return SparseMerkleTreeImpl;
}());
exports.SparseMerkleTreeImpl = SparseMerkleTreeImpl;
